          ...
          <tbody>
            {products && products.length > 0 ? (
              products.map((item, key) => {
                return (
                  // render single row for the table
                  <ProductTable
                    product={item}
                    editProduct={
                      (pId, product, type) => editProduct(pId, product, type)
                    }
...
const ProductTable = ({ product, deleteProduct, editProduct }) => {
  ...
  const handleProductQuantity = (product) => {
    if (product.isVariation) {
      // calculate total quantity from variations, as the array looks like this: [{size: "M", color: "red", quantity: 10}, {size: "L", color: "red", quantity: 10}]
      console.log("variation", product)
      let totalQuantity = 0;
      product.pVariations.forEach((variation) => {
        totalQuantity += parseInt(variation.quantity);
      });
  ...
  return (
    <Fragment>
      <tr>
        ...
        <td className="p-2 text-center">{handleProductQuantity(product)}</td>
        <td className="p-2 text-center">
