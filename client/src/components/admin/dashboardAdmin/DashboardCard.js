...
const DashboardCard = (props) => {
  ...
  const todayAllOrders = () => {
    let total = 0;
    let orders = data.totalOrders.Orders;
    if (orders) {
      orders.map((order) => {
        let today = new Date();
    ...
    setTodayOrders(total);
  };

  const todayAllCustomers = () => {
    let total = 0;
    let customers = [];
    let orders = data.totalOrders.Orders;
    console.log(orders);
    if (orders) {
      orders.map((order) => {
        ...
        if (
          ...
          (order.phone !== null || order.phone !== null)
        ) {
  ...
  return (
    <Fragment>
      ...
      <div className="m-4 grid grid-cols-1 md:grid-cols-4 row-gap-4 col-gap-4">
        <div className="flex flex-col justify-center items-center col-span-1 bg-white p-6 shadow-lg hover:shadow-none transition-all duration-300 ease-in border-b-4 border-opacity-0 hover:border-opacity-100 border-indigo-200">
